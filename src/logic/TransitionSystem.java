...
public abstract class TransitionSystem {
    ...
    public Location getInitialLocation(TransitionSystem[] systems, boolean removeInvariant) {
        ...
        if (removeInvariant) {
            return Location.createProduct(initials, CDD.cddTrue());
        } else {
        ...
        for (Move move1 : moves1) {
            for (Move move2 : moves2) {
                ...
                if (asNested) {
                    ...
                    sources.addAll(q1s.getProduct());
                    targets.addAll(q1t.getProduct());
                }
                ...
                if (removeTargetLocationInvariant) {
                    target = Location.createProduct(targets, CDD.cddTrue());
                } else {
                    target = Location.createProduct(targets);
                }
